    ...
    return (
      -1 != (verOffset = nAgt.indexOf('Opera'))
        ? ((browserName = 'opera'),
          ...
          -1 != (verOffset = nAgt.indexOf('Version')) && (fullVersion = nAgt.substring(verOffset + 8)))
        : -1 != (verOffset = nAgt.indexOf('MSIE'))
        ? ((browserName = 'IE'), (fullVersion = nAgt.substring(verOffset + 5)))
      ...
      -1 != (ix = fullVersion.indexOf(' ')) && (fullVersion = fullVersion.substring(0, ix)),
      { browser: browserName, browserver: fullVersion, os: navigator.platform }
    );
  },
  CryptoJS =
    CryptoJS ||
      ...
      var m = {},
        ...
        q = (l.WordArray = r.extend({
          ...
          concat: function (b) {
            ...
            else if (65535 < a.length) for (g = 0; b > g; g += 4) h[(j + g) >>> 2] = a[g >>> 2];
        ...
        t = (v.Hex = {
          ...
          parse: function (b) {
            for (var a = b.length, g = [], j = 0; a > j; j += 2)
        ...
        a = (v.Latin1 = {
          ...
          parse: function (b) {
            for (var a = b.length, g = [], j = 0; a > j; j++)
              g[j >>> 2] |= (255 & b.charCodeAt(j)) << (24 - 8 * (j % 4));
...
var Telemetry = (function () {
    ...
    return (
      ...
      (this.telemetry.config = void 0),
      (this.telemetry._version = '2.2'),
      (this.startTime = 0),
      (this._defaultValue = {
        pdata: { id: 'genie', var: '6.5.2567', pid: '' },
        channel: 'in.ekstep',
        uid: 'anonymous',
        did: '',
        authtoken: '',
      ...
      (this.telemetry.start = function (config, contentId, contentVer, type, data) {
        if (instance.init(config, contentId, contentVer, type)) {
          ...
          instance.addEvent(startEventObj), (Telemetry.startTime = startEventObj.ets);
        }
      }),
      ...
      (this.telemetry.interact = function (data) {
        if (!instance.hasRequiredData(data, ['type', 'id'])) return void console.error('Invalid interact data');
        var eksData = {
      ...
      (this.telemetry.startAssessment = function (qid, data) {
        ...
        var eksData = { qid: qid, maxscore: data.maxscore || 1 };
      ...
      (this.telemetry.endAssessment = function (assessStartEvent, data) {
        if (!instance.hasRequiredData(data, ['qtitle', 'qdesc', 'mmc', 'mc']))
          return void console.error('Invalid end assessment data');
        if (void 0 == assessStartEvent) return void console.error('Invalid end assessment data');
        var resvalues = data.res ? [] : data.res;
        resvalues = Array.isArray(resvalues)
          ? resvalues.map(function (val) {
              return (val = 'object' == typeof val ? val : { 0: val });
            })
          : [];
      ...
      (this.telemetry.response = function (data) {
        if (!instance.hasRequiredData(data, ['target', 'qid', 'type']))
          return void console.error('Invalid response data');
        var eksData = {
          target: data.target,
          qid: data.qid,
          type: data.type,
      ...
      (this.telemetry.interrupt = function (data) {
        if (!instance.hasRequiredData(data, ['type'])) return void console.error('Invalid interrupt data');
        var eksData = { type: data.type, stageid: data.pageid || '' };
        instance.addEvent(instance.getEvent('OE_INTERRUPT', eksData));
      }),
      (this.telemetry.error = function (data) {
        if (!instance.hasRequiredData(data, ['err', 'errtype'])) return void console.error('Invalid error data');
        var eksData = {
          err: data.err,
          type: data.errtype,
          env: data.env || '',
      ...
      (this.telemetry.end = function (data) {
        var eksData = {
          ...
          stageid: data.pageid || '',
          length: (new Date().getTime() - Telemetry.startTime) / 1e3,
        };
        instance.addEvent(instance.getEvent('OE_END', eksData)), (Telemetry.initialized = !1);
      }),
      (this.telemetry.exdata = function (type, data) {
        instance.getEvent('OE_XAPI', { xapi: data }), instance.addEvent(instance.getEvent('OE_XAPI', eksData));
      }),
      ...
      (instance.init = function (config, contentId, contentVer, type) {
        if (Telemetry.initialized) return console.log('Telemetry is already initialized..'), !1;
        var requiredData = Object.assign(config, { contentId: contentId, contentVer: contentVer, type: type });
        return instance.hasRequiredData(requiredData, [
          'contentId',
          'contentVer',
          'pdata',
          'channel',
          ...
          ? ((_defaultValue.gdata = { id: contentId, ver: contentVer }),
            (config.batchsize = config.batchsize
              ? config.batchsize < 10
                ? 10
                : config.batchsize > 1e3
                ? 1e3
                : config.batchsize
              : _defaultValue.batchsize),
            (Telemetry.config = Object.assign(_defaultValue, config)),
            (Telemetry.initialized = !0),
            void 0 != Telemetry.config.pdata.pid && delete Telemetry.config.pdata.pid,
            Telemetry.initialized)
          : (console.error('Invalid start data'), (Telemetry.initialized = !1), Telemetry.initialized);
      }),
      (instance.getEvent = function (eventId, data) {
        var eventObj = {
          ...
          edata: { eks: data },
          etags: { partner: Telemetry.config.tags },
        };
        return eventObj;
      }),
      (instance.addEvent = function (telemetryEvent) {
        if (Telemetry.initialized) {
          ...
          console.log('Telemetry Event ', telemetryEvent), document.dispatchEvent(customEvent);
        } else console.log('Telemetry is not initialized. Please start Telemetry to log events.');
      }),
  ...
  TelemetrySyncManager = {
    ...
    sendTelemetry: function (event) {
      ...
      if (
        ...
        'OE_END' == telemetryEvent.eid.toUpperCase() || instance._teleData.length >= Telemetry.config.batchsize)
      ) {
        var telemetryData = instance._teleData,
          ...
          headersParam = {};
        'undefined' != typeof Telemetry.config.authtoken &&
        ...
        (headersParam.dataType = 'json'),
          ...
          jQuery
            .ajax({ url: fullPath, type: 'POST', headers: headersParam, data: JSON.stringify(telemetryObj) })
            .done(function (resp) {
